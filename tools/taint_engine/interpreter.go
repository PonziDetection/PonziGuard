...
type ScopeContext struct {
	...
	Contract *Contract
	//taint mem stack
	Taint_mem	*TaintMemory
	Taint_stack	*TaintStack
}
...
type EVMInterpreter struct {
	...
	//return of CALL push into memory
	returnFlag []int
}
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, taintFlag []int, err error) {

/*
	if RecordFlag == true{
		file, err1 := os.OpenFile("/home/lrc/myproject/crush_interpreter.txt", os.O_RDWR|os.O_CREATE | os.O_APPEND, 0644)
	    if err1 != nil{
	    ...
	    fmt.Fprintf(file,"interpreter.run\n")
	...
	*/
	...
	for {
		...
		if in.cfg.Debug {
			...
			logged = true
			// track taint_stack and taint_mem
			taint_stack.JPrint()
			//taint memory track
			fmt.Println()
		...
		//add taintFlag
		res, taintFlag, err = operation.execute(&pc, in, callContext)
