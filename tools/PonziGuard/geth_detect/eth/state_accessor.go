...
func (eth *Ethereum) stateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool) (statedb *state.StateDB, err error) {
	...
	if base != nil {
		...
		/*for ; ; {
			...
			//statedb, err = state.New(current.Root(), database, nil)
			//if err != nil {
			//	log.Info("state.New() erro!", "erroblock", current.NumberU64())
			//}
			...
			if current.NumberU64() == 0{
				...
				//}
				//if err == nil {
				//	log.Info("state.New() successed!", "successedblock", current.NumberU64())
				//}
				break
				
			}
		}*/
		//parent := eth.blockchain.GetBlockByNumber(0)
		current = eth.blockchain.GetBlockByNumber(0)
		statedb, err = state.New(current.Root(), database, nil)
		log.Info("state.New() successed!", "successedblock", current.NumberU64())
	...
	for current.NumberU64() < origin {
		...
		if current.NumberU64() % 100000 == 0{
			log.Info("Processing blocks","block",current.NumberU64())
		}
